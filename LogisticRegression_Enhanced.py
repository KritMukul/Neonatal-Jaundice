"""
Model 1: Logistic Regression - ENHANCED VERSION
Improvements:
...
print("="*80)
print("MODEL 1: LOGISTIC REGRESSION - ENHANCED VERSION")
print("="*80)
...
print("\n" + "="*80)
print("LINEAR REGRESSION - BILIRUBIN PREDICTION")
print("="*80)

lr_reg = LinearRegression(fit_intercept=True)

cv_predictions = cross_val_predict(lr_reg, X_full, y_full_reg, cv=10)
cv_r = np.corrcoef(y_full_reg, cv_predictions)[0, 1]
cv_rmse = np.sqrt(mean_squared_error(y_full_reg, cv_predictions))

print(f"\n10-Fold CV: R={cv_r:.4f}, RMSE={cv_rmse:.4f}")

lr_reg.fit(X_full, y_full_reg)
y_test_pred_reg = lr_reg.predict(X_test)

test_r = np.corrcoef(y_test_reg, y_test_pred_reg)[0, 1]
...
print("\n" + "="*80)
print("LOGISTIC REGRESSION - JAUNDICE DETECTION (ENHANCED)")
print("="*80)
...
cv_pred_proba = cross_val_predict(
    lr_clf, X_full, y_full_clf, cv=skf, method='predict_proba', n_jobs=-1
)[:, 1]
...
lr_clf.fit(X_full, y_full_clf)
y_test_proba = lr_clf.predict_proba(X_test)[:, 1]
...
# Save models
with open('models/linear_regressor_enhanced.pkl', 'wb') as f:
    pickle.dump(lr_reg, f)
with open('models/linear_classifier_enhanced.pkl', 'wb') as f:
    pickle.dump(lr_clf, f)

# Save results
results = pd.DataFrame({
    'Model': ['Linear_Regression', 'LogisticRegression_Classification_Enhanced'],
...
results.to_csv('results/LogisticRegression_Enhanced_results.csv', index=False)

# Save hyperparameters
hyperparams_df = pd.DataFrame([best_params])
hyperparams_df.to_csv('results/LogisticRegression_Enhanced_hyperparameters.csv', index=False)

print("✅ Models saved to models/")
print("✅ Results saved to results/LogisticRegression_Enhanced_results.csv")
print("✅ Hyperparameters saved to results/LogisticRegression_Enhanced_hyperparameters.csv")

print("\n" + "="*80)
print("LOGISTIC REGRESSION ENHANCED TRAINING COMPLETE!")
print("="*80)
print(f"\nSummary:")
print(f"  Regression:")
