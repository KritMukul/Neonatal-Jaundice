"""
Model 6: XGBoost - ENHANCED VERSION
Improvements:
...
print("="*80)
print("MODEL 6: XGBOOST - ENHANCED VERSION")
print("="*80)
...
print("\n" + "="*80)
print("XGBOOST REGRESSION - BILIRUBIN PREDICTION")
print("="*80)

xgb_reg = XGBRegressor(n_estimators=1000, max_depth=6, random_state=42, n_jobs=-1)

cv_pred = cross_val_predict(xgb_reg, X_full, y_full_reg, cv=10, n_jobs=-1)
...
print("\n" + "="*80)
print("XGBOOST CLASSIFICATION - JAUNDICE DETECTION (ENHANCED)")
print("="*80)
...
xgb_clf = XGBClassifier(**best_params, random_state=42, n_jobs=-1, eval_metric='logloss')
...
cv_pred_proba = cross_val_predict(
    xgb_clf, X_full, y_full_clf, cv=skf, method='predict_proba', n_jobs=-1
)[:, 1]
...
# Save models
with open('models/xgb_regressor_enhanced.pkl', 'wb') as f:
    pickle.dump(xgb_reg, f)
with open('models/xgb_classifier_enhanced.pkl', 'wb') as f:
    pickle.dump(xgb_clf, f)

# Save results
results = pd.DataFrame({
    'Model': ['XGBoost_Regression', 'XGBoost_Classification_Enhanced'],
...
results.to_csv('results/XGBoost_Enhanced_results.csv', index=False)

# Save hyperparameters
hyperparams_df = pd.DataFrame([best_params])
hyperparams_df.to_csv('results/XGBoost_Enhanced_hyperparameters.csv', index=False)

print("✅ Models saved to models/")
print("✅ Results saved to results/XGBoost_Enhanced_results.csv")
print("✅ Hyperparameters saved to results/XGBoost_Enhanced_hyperparameters.csv")

print("\n" + "="*80)
print("XGBOOST ENHANCED TRAINING COMPLETE!")
print("="*80)
print(f"\nSummary:")
print(f"  Regression:")
