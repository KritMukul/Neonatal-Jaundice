"""
Model 7: Multi-Layer Perceptron (MLP) - ENHANCED VERSION
Improvements:
...
print("="*80)
print("MODEL 7: MULTI-LAYER PERCEPTRON (MLP) - ENHANCED VERSION")
print("="*80)
...
print("\n" + "="*80)
print("MLP REGRESSION - BILIRUBIN PREDICTION")
print("="*80)
...
print("\n" + "="*80)
print("MLP CLASSIFICATION - JAUNDICE DETECTION (ENHANCED)")
print("="*80)
...
mlp_clf = MLPClassifier(**best_params, random_state=42)
...
cv_pred_proba = cross_val_predict(
    mlp_clf, X_full, y_full_clf, cv=skf, method='predict_proba', n_jobs=-1
)[:, 1]
...
# Save models
with open('models/mlp_regressor_enhanced.pkl', 'wb') as f:
    pickle.dump(mlp_reg, f)
with open('models/mlp_classifier_enhanced.pkl', 'wb') as f:
    pickle.dump(mlp_clf, f)

# Save results
results = pd.DataFrame({
    'Model': ['MLP_Regression', 'MLP_Classification_Enhanced'],
...
results.to_csv('results/MLP_Enhanced_results.csv', index=False)

# Save hyperparameters
hyperparams_df = pd.DataFrame([best_params])
hyperparams_df.to_csv('results/MLP_Enhanced_hyperparameters.csv', index=False)

print("✅ Models saved to models/")
print("✅ Results saved to results/MLP_Enhanced_results.csv")
print("✅ Hyperparameters saved to results/MLP_Enhanced_hyperparameters.csv")

print("\n" + "="*80)
print("MLP ENHANCED TRAINING COMPLETE!")
print("="*80)
print(f"\nSummary:")
print(f"  Regression:")
