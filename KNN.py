...
print("="*70)
print("MODEL 2: K-NEAREST NEIGHBOR (k=5)")
print("="*70)
...
cv_pred = cross_val_predict(knn_reg, X_full, y_full_reg, cv=10)
...
cv_pred_clf = cross_val_predict(knn_clf, X_full, y_full_clf, cv=10)
knn_clf.fit(X_full, y_full_clf)
cv_pred_proba = cross_val_predict(knn_clf, X_full, y_full_clf, cv=10, method='predict_proba')[:, 1]
...
y_test_pred_clf = knn_clf.predict(X_test)
y_test_proba = knn_clf.predict_proba(X_test)[:, 1]
...
with open('models/knn_regressor.pkl', 'wb') as f:
    pickle.dump(knn_reg, f)
with open('models/knn_classifier.pkl', 'wb') as f:
    pickle.dump(knn_clf, f)

results = pd.DataFrame({
    'Model': ['KNN_Regression', 'KNN_Classification'],
