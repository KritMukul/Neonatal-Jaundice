...
for f,(tr,val) in enumerate(tqdm(list(kf.split(X,y_clf)))):
    ...
    clf = RandomForestClassifier(n_estimators=200).fit(X[tr],y_clf[tr])
    probs = clf.predict_proba(X[val])[:,1]
    preds = clf.predict(X[val])

    reg = RandomForestRegressor(n_estimators=200).fit(X[tr],y[tr])
...
.to_csv("results/randomforest.csv",index=False)