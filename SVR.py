...
# REGRESSION
print("\n" + "="*70)
print("SVR - BILIRUBIN PREDICTION")
print("="*70)

svr = SVR(kernel='rbf')

cv_pred = cross_val_predict(svr, X_full, y_full_reg, cv=10)
...
cv_pred_clf = cross_val_predict(svc, X_full, y_full_clf, cv=10)
svc.fit(X_full, y_full_clf)
cv_pred_proba =cross_val_predict(svc, X_full, y_full_clf, cv=10, method='predict_proba')[:, 1]
...
y_test_pred_clf = svc.predict(X_test)
...
results.to_csv('results/SVR_results.csv', index=False)
