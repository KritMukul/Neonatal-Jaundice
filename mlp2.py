...
for f,(tr,val) in enumerate(tqdm(list(kf.split(X,y_clf)))):
    ...
    clf = MLPClassifier(hidden_layer_sizes=(128,64), max_iter=300).fit(X[tr],y_clf[tr])
    probs = clf.predict_proba(X[val])[:,1]
    preds = clf.predict(X[val])

    reg = MLPRegressor(hidden_layer_sizes=(128,64), max_iter=300).fit(X[tr],y[tr])
...
.to_csv("results/mlp.csv",index=False)