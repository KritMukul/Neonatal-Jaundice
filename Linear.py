...
- Linear Regression with default hyperparameters (fit_intercept=True)
- 10-Fold Cross-Validation
- Regression: Predict bilirubin (TsB)
- Classification: TsB > 15 mg/dL = jaundice (1), else healthy (0)
...
cv_mae = mean_absolute_error(y_full_reg, cv_predictions)

print(f"\n10-Fold Cross-Validation:")
print(f"  Correlation (R): {cv_r:.4f}")
...
# CLASSIFICATION
print("\n" + "="*70)
print("LOGISTIC REGRESSION - JAUNDICE DETECTION")
print("="*70)
...
# 10-Fold CV
cv_pred_clf = cross_val_predict(lr_clf, X_full, y_full_clf, cv=10)
lr_clf.fit(X_full, y_full_clf)
cv_pred_proba = cross_val_predict(lr_clf, X_full, y_full_clf, cv=10, method='predict_proba')[:, 1]
...
# Test
y_test_pred_clf = lr_clf.predict(X_test)
...
results = pd.DataFrame({
    'Model': ['Linear_Regression', 'Linear_Classification'],
...
results.to_csv('results/Linear_results.csv', index=False)
